...
const UserPreferencePage: React.FC = () => {
  ...
  // Keep interests in local state only, completely separate from Redux until debounced update
  const [localInterests, setLocalInterests] = useState(preferences.interests || '');

  // Create a stable reference to the debounce function
  const debouncedUpdateInterests = useRef(
    debounce((value: string) => {
      dispatch(updatePreferences({ interests: value }));
    }, 800) // Increased debounce time to reduce Redux updates
  ).current;
  ...
  // Update local interests when preferences change from external source
  useEffect(() => {
    // Only update if they're different to avoid unnecessary renders
    if (preferences.interests !== localInterests) {
      setLocalInterests(preferences.interests || '');
    }
  ...
  useEffect(() => {
    return () => {
      debouncedUpdateInterests.cancel();
    };
  }, [debouncedUpdateInterests]);

  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    const { name, value } = e.target;

    // Special handling for interests to prevent lag
    if (name === 'interests') {
      setLocalInterests(value); // Update local state immediately
      debouncedUpdateInterests(value); // Debounce the Redux update
    } else {
      // For all other inputs, update Redux state directly
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // Make sure the latest interests are in Redux before submitting
    dispatch(updatePreferences({ interests: localInterests }));
    dispatch(fetchItinerary({ destination, preferences: { ...preferences, interests: localInterests } }));
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-lg p-6 w-full max-w-md shadow-lg">
        ...
        <form onSubmit={handleDestinationSubmit} className="space-y-4">
          <input
            ...
            className="w-full p-3 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-gray-900 dark:text-white"
            required
          />
          <button
            type="submit"
  ...
  const PreferenceForm = () => (
    <div className="max-w-3xl mx-auto">
      <div className="bg-white dark:bg-zinc-900 rounded-xl shadow-lg p-8 transition-all duration-200">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Who Are You Traveling With?</label>
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center">
              ...
              />
              <span className="ml-4 text-lg font-semibold w-8 text-center text-gray-800 dark:text-white">{preferences.days}</span>
            </div>
          </div>

          {/* Budget Level */}
          <div className="space-y-2">
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Budget Level</label>
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                >
                  ...
                  <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{budget.label}</span>
                </label>
              ))}
            </div>
          </div>
          ...
          {/* Pace */}
          <div className="space-y-2">
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Pace</label>
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                >
                  ...
                  <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{pace.label}</span>
                </label>
              ))}
            </div>
          </div>
          ...
          {/* Transportation */}
          <div className="space-y-2">
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Transportation Preference</label>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                ...
                >
                  ...
                  <span className="text-xs font-medium text-center text-gray-700 dark:text-gray-300">{transport.label}</span>
                </label>
              ))}
            </div>
          </div>
          ...
          {/* Accommodation */}
          <div className="space-y-2">
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Accommodation Type</label>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                ...
                >
                  ...
                  <span className="text-xs font-medium text-gray-700 dark:text-gray-300">{accom.label}</span>
                </label>
              ))}
            </div>
          </div>

          {/* Interests - FIXED: Using local state completely separate from Redux until form submission */}
          {/* <div className="space-y-2">
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Your Interests and Preferences</label>
            <textarea
              name="interests"
              ...
              className="w-full p-3 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 h-24 transition-colors duration-200 text-gray-900 dark:text-white"
            />
          </div> */}
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-xl shadow-lg overflow-hidden transition-all duration-200">
        ...
        <div className="p-6">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white">{itinerary[activeDay].title}</h2>
            ...
            {itinerary[activeDay].activities.map((activity: Activity, actIndex: number) => (
              <div key={actIndex} className="bg-gray-50 dark:bg-gray-700/50 rounded-lg overflow-hidden">
                <div className="flex flex-col md:flex-row">
                  {activity.imageUrl && (
                    <div className="md:w-1/3 h-48 md:h-auto overflow-hidden">
                      <img
                        ...
                        alt={typeof activity.title === 'string' ? activity.title : 'Activity image'}
                  ...
                  <div className={`p-6 ${activity.imageUrl ? 'md:w-2/3' : 'w-full'}`}>
    ...
    <main className="container mx-auto px-4 py-8">
      <header className="mb-8 text-center">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">
          {showItinerary ? `Your Trip to ${destination}` : `Plan Your Trip to ${destination}`}
        </h1>
        <p className="text-gray-600 dark:text-gray-300">
          {showItinerary
            ? 'Review your custom itinerary based on your preferences.'
            : 'Customize your travel preferences to get a personalized itinerary.'}
        </p>
      </header>

      {showDestinationModal && <DestinationModal />}
      {showItinerary ? <ItineraryView /> : <PreferenceForm />}
    </main>
  );
};
